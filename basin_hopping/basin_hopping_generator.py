...
# @Last modified time: 2025-05-11 10:23:14
...
from xyz_physical_geometry_tool_mod import is_physical_geometry, validate_structure
from gamess_tools import gamess_vibmin
from xyz_tools import molecule_stat
from gaussian_tools import gaussian_job
...
class BasinHoppingGenerator(BaseGenerator):
    ...
    def __init__(self,
                 ...
                 optimize_interface: str = "gaussian", # "ase", "gamess_dftb", "gaussian"
        ...
        Args:
            ...
            energy_outliner_threshold: Used for detect the undesired local minima which is very often is the broken structure.
                                        This kind of structure normally has very low energy,
                                        hence, it may cause the BH to abnormally terminated
        """
        ...
        #self.attach_angles = [(0, 0), (0, 120), (0, 240), (120, 0), (120, 120), (120, 240), (240, 0), (240, 120), (240, 240)]  # 3 grid points for rotation after attach (in degrees)
        ...
        self.optimize_interface = optimize_interface
        ...
        self.nnp_model = None
        if "state_dict" in self.model_params and self.optimize_interface == "ase":
    ...
    def __call__(self, n_steps: int = 100) -> MolecularStructure:
        ...
        while step < n_steps:
            ...
            optimized_structure, optimized_energy, trajectory_path = self._optimize_structure_gaussian(proposed_structure)
    ...
    def _initialize(self):
        ...
        attempts = 0
        while structure is None and attempts < 50:
            grid_point = self._get_random_grid_point()
            structure = self._apply_operations(grid_point)
            attempts += 1
        ...
        optimized_structure, optimized_energy, trajectory_path = self._optimize_structure_gaussian(structure)
    ...
    def _optimize_structure_gaussian(self, structure: MolecularStructure) -> Tuple[Optional[MolecularStructure], float]:
        """Optimize a structure using the NNP model.
        ...
        Returns:
            Tuple of (optimized structure, energy)
        """
        header_str = "%mem=2GB\n%nprocshared=1\n#opt(nomicro,maxcyc=500) nosymmetry external ='run.py g09_external_schnet_func.json {}'\n\n_\n\n1 1\n"

        if not self.model_params["state_dict"]:
            raise ValueError("The best_model path is not defined")
        elif os.path.basename(self.model_params["state_dict"]) != "best_model":
            raise ValueError("Current implementatation only support 'best_model' only")

        opt_pref = {
            "convert_to_xyz": True, "convert_frame_list": [-1],
            "header": header_str.format(self.model_params["state_dict"]),
            "preserve_out": False}
    ...
    def _optimize_structure_dftb(self, structure: MolecularStructure) -> Tuple[Optional[MolecularStructure], float]:
        """Optimize a structure using the NNP model.
        ...
        Returns:
            Tuple of (optimized structure, energy, trajectory)
        """

        try:
            xyz_coor = structure.to_xyz_list()
            gamess_opt = gamess_vibmin(xyz_coor,
                                        parm_loc="/home/htphan/src/gamess/parameters",
                                        gamess_loc="/home/htphan/src/gamess/rungms",
                                        molecular_charge=0,
                                        preserve_inp=False)

            optimized_structure = MolecularStructure.from_xyz_list(gamess_opt.get("xyz_list"))
            optimized_structure.metadata = structure.metadata.copy()
    ...
    def _apply_operations(self, grid_point: Tuple) -> Optional[MolecularStructure]:
        ...
        try:
            ...
            if seed_idx >= len(self.seed_structures):
    ...
    def _append_to_output_file(self, structure: MolecularStructure, energy: float, step: int, accept: bool):
        ...
        try:
            ...
            comment_parts = [f"Step= {step}", f"eng= {energy:.6f}", f"GridPoint= {grid_point}"] + operations_info
    ...
    def _get_operations_description(self, grid_point: Tuple) -> Dict[str, Any]:
        ...
        for op_idx, op_type in enumerate(self.operation_sequence):
            if op_type == OperationType.FLIP:
                ...
                attach_angle_idx = grid_point[grid_idx]
                grid_idx += 1
                #print(f"attach_angle_idx: {attach_angle_idx}")
                #print(f"self.attach_rotate_generator.params['angle_list']: {self.attach_rotate_generator.params['angle_list']}")
    ...
    def set_seed_structure(self, seed_structures):
        """Set the seed structure."""
        if isinstance(seed_structures, str):
    ...
    def set_attach_params1(self, params):
        """ Set attach params"""
        self.attach_rotate_generator.set_params(**params)
